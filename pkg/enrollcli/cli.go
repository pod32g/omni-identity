// Package enrollcli is the omni-enrollment command line as an importable
// package, so another program (the Omni Access desktop client) can ship the
// endpoint agent inside its own executable and re-execute itself with these
// arguments instead of bundling a second binary. cmd/omni-enrollment is a
...
// Version is reported by `version`; set it before calling Main.
var Version = "0.1.0-dev"

const defaultConfigPath = "/etc/omni-enrollment/config.yaml"

// Main runs the CLI with argv (the arguments after the program name)
...
func Main(argv []string) int {
	...
	if len(argv) < 1 || strings.HasPrefix(argv[0], "-") && !isHelpOrVersion(argv[0]) {
		if err := runGUI(argv); err != nil {
